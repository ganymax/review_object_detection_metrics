...
import numpy as np
from src.bounding_box import BBFormat, BoundingBox, COCO_SMALL_AREA_THRESHOLD, COCO_LARGE_AREA_THRESHOLD
from src.utils.enumerators import ObjectScale

# COCO scale area ranges
SCALE_AREA_RANGES = {
    ObjectScale.SMALL: (0, COCO_SMALL_AREA_THRESHOLD),
    ObjectScale.MEDIUM: (COCO_SMALL_AREA_THRESHOLD, COCO_LARGE_AREA_THRESHOLD),
    ObjectScale.LARGE: (COCO_LARGE_AREA_THRESHOLD, np.inf),
}
...
def get_scale_metrics(groundtruth_bbs, detected_bbs, iou_threshold=0.5, max_dets=100):
    """ Calculate metrics for each COCO scale category (small, medium, large).
    ...
    -------
    dict
        A dictionary with scale categories as keys containing:
        - count: number of ground truth boxes in this scale
        - AP: average precision for this scale
        - AR: average recall for this scale
        - TP: true positives
        - FP: false positives
        - area_range: the (min, max) area range for this scale
    """
    _bbs = _group_detections(detected_bbs, groundtruth_bbs)
    _ious = {k: _compute_ious(**v) for k, v in _bbs.items()}

    def _evaluate_scale(area_range):
        _evals = defaultdict(lambda: {"scores": [], "matched": [], "NP": []})
        for img_id, class_id in _bbs:
            ev = _evaluate_image(
                _bbs[img_id, class_id]["dt"],
        ...
        for class_id in _evals:
            acc = _evals[class_id]
            acc["scores"] = np.concatenate(acc["scores"]) if acc["scores"] else np.array([])
            acc["matched"] = np.concatenate(acc["matched"]).astype(np.bool) if acc["matched"] else np.array([])
            acc["NP"] = np.sum(acc["NP"])
    ...
    for scale in ObjectScale:
        area_range = SCALE_AREA_RANGES[scale]
        results = _evaluate_scale(area_range)

        gt_count = len(BoundingBox.get_bounding_boxes_by_scale(groundtruth_bbs, scale))
        det_count = len(BoundingBox.get_bounding_boxes_by_scale(detected_bbs, scale))

        ap_values = [x['AP'] for x in results if x['AP'] is not None]
        ar_values = [x['TP'] / x['total positives'] for x in results if x['TP'] is not None]
        tp_values = [x['TP'] for x in results if x['TP'] is not None]
        fp_values = [x['FP'] for x in results if x['FP'] is not None]

        scale_metrics[scale] = {
            'gt_count': gt_count,
            'det_count': det_count,
            'AP': np.mean(ap_values) if ap_values else np.nan,
            'AR': np.mean(ar_values) if ar_values else np.nan,
            'TP': sum(tp_values) if tp_values else 0,
            'FP': sum(fp_values) if fp_values else 0,
...
def get_dataset_scale_profile(bounding_boxes):
    """ Get a comprehensive profile of the dataset based on COCO scale categories.

    Parameters
    ----------
    bounding_boxes : list
        A list containing objects of type BoundingBox.

    Returns
    -------
    dict
        A dictionary containing:
        - total_count: total number of bounding boxes
        - scale_distribution: count per scale category
        - scale_statistics: detailed statistics per scale
        - scale_percentages: percentage distribution
    """
    total_count = len(bounding_boxes)
    scale_distribution = BoundingBox.get_scale_distribution(bounding_boxes)
    scale_statistics = BoundingBox.get_scale_statistics(bounding_boxes)

    scale_percentages = {}
    for scale in ObjectScale:
        count = scale_distribution[scale]
        scale_percentages[scale] = (count / total_count * 100) if total_count > 0 else 0
